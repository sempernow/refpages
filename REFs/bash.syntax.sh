...
# PROCESS SUBSTITUTION (PS) Operators
    ...
    # STDOUT to FILE
    command2 <(command1)  # command2 takes FILE (arg); connects per /dev/fd/# 
    
    # STDOUT to STDIN 
    command1 >(command2)  # command2 takes STDIN (arg); useful to AVOID SUBSHELL of pipe

    # Redirect of Process Subsitition: 
    # Use where command2 argument must be a file,
    # and command1 generates and writes such content to STDOUT.
    command2 < <(command1)
...
# PIPELINES & REDIRECTION Operators 
    ...
    ## REDIRECTION : stdin (0), stdout (1), stderr (2), i.e., NAME (FD)
        ...
        # DUPLICATING FDs (FILE is path to a file)
        ...
        # OPENING FDs for Reading and Writing
            ...
            command1 < FILE      # Redirect a file to stdin of command1
            command1 > FILE      # Redirect stdout of command1 to FILE
            command1 > FILE 2>&1 # Redirect stdout & stderr of command1 to FILE (good)
            command1 &> FILE     # Redirect stdout & stderr of command1 to FILE (bad)
            command1 2> FILE     # Redirect stderr to a file
...
# COMPOUND COMMANDS [BUILT-IN STUCTURES] 
    ...
    # CONDITIONAL constructs 
        ...
        # REPLACE a `for` loop with PIPELINE per `xargs`
