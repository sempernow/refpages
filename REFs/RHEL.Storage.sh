...
# UTILITIES 
    ...
    # FILESYSTEM USE/MONITOR
        ...
        du -h         # disk usage [used-size]; per FOLDER; human-readable [-h]
        du -h ROOTDIR # size [used], per folder, @ ROOTDIR
        du -sh        # summarize only; total folder/subfolder size [used]
        du -sh *      # summarize each [1st-child-folder + root files]
    ...
    # lsblk :: list block devices
        lsblk -o SIZE,LABEL,TYPE,NAME,UUID # the most useful info
        lsblk -o SIZE,LABEL,TYPE,NAME,UUID | grep 'Scratch' # @ 'Scratch' [label]
    ...
    # DISK QUOTAs [per fs] quota, quotaoff/on, quotacheck, edquota, repquota
        ...
        /dev/sdb1  /mnt/FOLDERNAME  ext3  defaults,usrquota,grpquota  0  2
        # then REMOUNT; unmount/mount device
        umount /mnt/FOLDERNAME; mount /mnt/FOLDERNAME
        quotaoff /mnt/FOLDERNAME # safety; even though no quota files yet.
        quotacheck -cug /mnt/FOLDERNAME # create quota files for user+group
        ...
        quotaon /mnt/FOLDERNAME
        repquota # report quotas 
        repquota /mnt/FOLDERNAME

    # SYMLINKS MAP each device UUID to its '/dev' entry
        /dev/disk/by-uuid
...
# MOUNT FILESYSTEM
    ...
    # mount block device 'sdb1', of FS type Ext3, @ mount-point '/mnt/foo' (path must exist)
    ...
    # mount a LOOP DEVICE; a file as a block device  https://en.wikipedia.org/wiki/Loop_device  
        mkdir /mnt/bar 
        mount [-t ext4] -o loop foo.iso /mnt/bar
    ...
    # @ 'fstab' (AUTOMOUNT on boot) ('Legacy', but also 'PREFERRED') 
        ...
        vim /etc/fstab  # man fstab(5) http://man7.org/linux/man-pages/man5/fstab.5.html   
        ...
        mount -a  # (re)mount all per /etc/fstab
    ...
    # Note: 'fstab' is still effective AND takes precedent, even though "legacy"
        ...
        # automount requires UNIT FILE named:
            ...
            # EITHER (mount/automount) can be ACTIVE per systemctl.
        ...
        # NAMING CONVENTION (REQUIRED)
            ...
            # USAGE 
                ...
                lv${_NAME}  # find per lsblk command

            # E.g., Create automount of a USB drive, per Unit Files
                vim /etc/systemd/${_NAME}.mount
                "
                ...
                "
                vim /etc/systemd/${_NAME}.automount
                "
                    [Unit]
...
# FILESYSTEMs
    ...
    # CIFS/Samba (Common Internet File System); successor to SMB ("Samba") Protocol
        ...
        # Create a CIFS/Samba share (replace uid/gid number[s]w/ apropos)
            vim /etc/fstab  # (see 'MOUNT' section for systemd method)
            # 1. edit fstab, e.g., 
...
# https://en.wikipedia.org/wiki/File_system_permissions
    ...
    # SPECIAL PERMISSIONS :: SUID, SGID, Sticky-Bit
        ...
        # SUID [Set User ID] if bit set, user runs FILE with owner's permissions
            # Used on SOME SYSTEM executables; DO NOT set it for any [other] file or folder
            chmod u+s FILE # SUID; owner: 'rwx' => 'rws'

        # SGID/GUID [Set Group ID] if bit set, FILE has group permissions
            # Used on folders 
            chmod g+s FILE # GUID; group: 'rwx' => 'rws'

        # Sticky-Bit [Set Other ID]; only owner or root can delete or move
...
# ACLs [Access Control List]s :: Newest File-based Permissions scheme
    ...
    # set GROUPname perms for all EXISTing @ FOLDERname ...
    setfacl -m g:GROUPname:rx FOLDERname # [-m]odify; [g:]roup GROUPname; set perms, to 'rx', @ FOLDERname 
    setfacl -R -m ... # recurse; apply to all subdirs too; do NOT use '-R' with 'd:' [see below]
    
    # set GROUPname perms for all FUTURE @ FOLDERname ... [default] 
    setfacl -m d:g:GROUPname:rx FOLDERname # ... [d:]efault ...
