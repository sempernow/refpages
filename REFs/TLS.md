...
TLS/SSL is a trust-based scheme. The server sends a cryptographically-generated certificate, and the client application validates that against the Certificate Authority (CA) certificate (root certificate) from which the server's certificate claims to be created. Nominally, the client application obtains that root CA certificate from the default store at its parent OS. Some applications add their own stores, options, and such to that.
...
>**On TLS-handshake failure**, the server is likely to send a "handshake failure" alert. Such may occur if the certificate's CA cannot be validated by the client, or if client and server have no mutually supported TLS-ciphers suite. A TLS alert message is a protocol-level message that indicates an issue during the TLS handshake or data exchange. The specific alert code sent by the server can provide more details about the nature of the problem. In the case of a lack of common cipher suites, the server is likely to send a "handshake failure" alert. The TLS handshake failure is not an HTTP-level response; it's part of the TLS protocol. The HTTP layer comes into play only after a secure TLS connection has been established. If the TLS handshake fails, the HTTP layer doesn't have an opportunity to send an HTTP response code because the connection hasn't been established.
...
```bash
cn='foo.local'            # Common Name AKA Domain Name
root_ca_crt='root-ca.crt' # Root Certificate-Authority Certificate

openssl s_client -connect $cn:443 -CAfile $root_ca_crt 2>&1 |tee "openssl.s_client.connect.${cn}.cafile.log"
```
...
### [`openssl`](https://www.openssl.org/) | [`REF.TLS.openssl.sh`](REF.TLS.openssl.sh)

The multi-purpose OpenSSL tool used to create, view, and test TLS keys, certificates, and such.
Also useful for TLS at `https://localhost` (and aliases) for development. **Satisfies all browsers**.  
...
Get cert per automated DNS-01 challenge
...
If the AWS-credentialed bot-user (@ `acme.sh`) hasn't the proper AWS role/policy 
required to perform the DNS-01 challenge, then &hellip;
...
... 
AccessDenied
User: arn:aws:iam::971722315751:user/acme is not authorized to perform: route53:ListHostedZones
</ErrorResponse>
[Wed Jul 28 21:27:24 UTC 2021] invalid domain
[Wed Jul 28 21:27:26 UTC 2021] Error add txt for domain:_acme-challenge.example.com
