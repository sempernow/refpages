...
# Search for chart against all locally-cached repos
helm search repo $app_or_keyword  # --versions, -l for all versions

# List Helm's environment 
helm env

# Add repo of ArtifactHUB.io 
helm repo add hub $url

# Update repos list : pull newer-version references into local cache
helm repo update

# List repos added (to local cashe)
helm repo list

# List installed charts (releases)
helm list  # -A, --all-namespaces

# Show chart info by object : chart, values, readme, crds, all
helm show $object $chart

# Install/Upgrade a remote chart : $release may be any name
helm upgrade --install $release $chart
## Options useful on chart install/upgrade
    --version $ver      # Force specific version
    --generate-name     # Instead of $release, auto-generate a release name
    --create-namespace  # if not already by `k create ns $ns`
    --namespace $ns     # Then `helm -n $ns ...`
    -f $values          # Override chart's values.yaml with this modified one.
    --atomic            # Teardown on fail (pv/pvc requires MANUAL teardown).
    --debug             # Always
    --dry-run           # Always on first try, then remove if okay.

# Install/Upgrade a local (pulled) chart from its $extracted folder 
## 1. Pull chart
helm pull $chart --version $ver # Dumps to ./$tarball file 
tar -xaf $tarball               # Extracts to ./$extracted folder
## 2. Install/Upgrade the local chart
helm upgrade --install $release $extracted 

# Install/Upgrade a local (pulled) chart from its $extracted folder using modified $values file
## 1. Pull chart
helm pull $chart --version $ver # Dumps to ./$tarball file 
tar -xaf $tarball               # Extracts to ./$extracted folder
## 2. Extract chart's values.yaml file.
helm show values $chart |tee $values
...
helm upgrade --install -f $values $release $extracted 

# Get useful info on the installed chart (release); also prints "Notes: ..." of the install 
helm status $release
...
```
- **Avoid using the "`helm install $release $chart`" method*** of installing a chart, though it is the "Getting started" way to do it. That method does not allow for subsequent upgrades without first deleting the existing (deployed) release.
