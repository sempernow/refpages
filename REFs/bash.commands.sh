...
# VIRTUAL CONSOLEs
    ...
    who # @ bash in tty1|tty4 after log in as root @ tty4
        ...
        uZer     tty1         2017-01-20 10:44 (:0)
        uZer     pts/0        2017-01-20 12:35 (:0.0)
        uZer     pts/1        2017-01-20 12:59 (:0.0)
...
# ENVIRONMENT VARIABLES (useful ones)
    ...
    # Prompt String 1; Primary command prompt definition. 
    ...
    # useful options @ sourced script ...
    set -a              # EXPORT ALL     
    set +a              # END export all 
    set -o posix        # do NOT print funcs on `set` command; run once
    ...
    export -n varName   # Un-export varName; remove from Env.
    export -nf funcName # Un-export funcName; remove from Env.
    env –i [Var=Value]  # CLEAR Env. Var.; ALL if no specifier(s)
    env –i bash         # new shell clear of all Env. Vars.; prior environment restored on `exit`
    unset  VARname      # clear an Env. Var.
    ...
    # Bash Config Files (Environment Variables) 
        ...
        .bashrc  # may be several, at several locations; applied per hierarchy ...
            ...
            # E.g., source a protected [755] config script [yet available to all users] 
                echo source /home/uZer/etc/foo.cfg > /etc/profile.d/foo.custom.sh
...
# PROCESS MANAGEMENT  >>>  See 'REF.RHEL.SysAdmin.sh' 

# STORAGE : BLOCK DEVICEs  >>>  See 'REF.RHEL.Storage.sh'  
    ...
    # COPY BLOCK DEVICES  https://wiki.archlinux.org/index.php/disk_cloning 
        ...
        # copy .img to USB 
        dd if=fname.img of=/dev/da0 bs=1M conv=sync
        ...
        dd if=/dev/urandom bs=1024 count=1 of=ASCII.dd.dev.urandom.1024
...
# STORAGE : FOLDERs / FILEs 
    ...
    mv PATH toPATH  # move/rename file or folder [DANGEROUS] 
    mv FILE DIR/    # move file to another folder

    # RENAME with WILDCARDs / SERIALIZE 
        ...
        # rename : SERIALIZE/sequential : (filter) | (build rename command) | (run per bash)
    ...
    # DELETE (REMOVE) FILEs|FOLDERs
        rm FILE     # remove/delete [DANGEROUS]
        rm /foo/bar/{fname1,fname2,fname3} # remove three files, per glob, per fname.
        ... |xargs rm  # remove a NEWLINE-delimited list of (verified) target files
        rm -i FILE  # interactive/prompt: -i
        rm -r DIR   # recursively remove folder and all subfolders and files therein
        rm -rf DIR  # same, forcibly

        # remove multiple files, but allow user VERIFY before delete ...
            ls |fgrep -i ".csv"     #=> dns-20161113-131146.csv 
            rm $(!!)           # run previous command, which is ...
            rm $(ls |fgrep -i ".csv")  # deletes filtered list of files
        ...
        # set target-file date to that of source file; reset mtime timestamp
        touch -r  REF TARGET          # per ref file/folder
        touch -rh REF_SYMLINK TARGET  # per ref symlink
        touch -t MMDDhhmm TARGET      # per MMDDhhmm [time format]
...
#
    ...
    # Using `find` http://mywiki.wooledge.org/UsingFind
        ...
        # RUN COMMANDS 
        #  Delimiters `;` and `+` MUST be ESCAPED|QUOTED
        # https://www.gnu.org/software/findutils/manual/html_mono/find.html#Run-Commands
            ...
            # MULTIPLE FILES, `\+` 
            #  `{}` is LIST of FILEs in process; must be at END; only ONE instance
            #  MUCH FASTER; true pipeline; inherently handles as many per thread as possible
            
                find ... -exec[dir] command args '{}' [args] \+
                find ... -exec[dir] command args '{}' [args] '+'  # equivalent

                # invoke subshell ... note: `\+` or `+`  [no escape needed] !?!?
                find ... -exec[dir] /bin/bash -c 'command "$@" ... "${@}"' _ {} \+
                    # E.g., 
                    find -exec /bin/bash -c 'printf "\t%s\n" "${@##*/}"' _ {} \+
                    # note `_` can be any string; it "consumes" bash subshell name, $0

                # this handles whitespace in file-path [inserts nul separator only]
                find ... -print0 |xargs -0 command 
                # e.g., handle one arg/path per line
                find -print0 |xargs -0 -I {} command {}
                
                # better than xargs method if command handles arg parsing  ...
                find ... -exec /bin/bash -c 'command' _ {} \+     

                # best [more secure, but slower]
                find ... -execdir /bin/bash -c 'command' _ {} \+  

                    # remove prepending `./` [if one path per line]
                    find ... |sed -n 's|./||p'    # WTF???
        ...
        # comma operator : multiple s in one traversal [each must incl any: -name, -type, ...]
        ...
        find . \( -execdir command {} \+ \) , \( -exec command {} \+ \) # multiple files per command call
            ...
            find "$_WIN_IT_PATH" \( -name 'REF.*' ! -iname '*.lnk' ! -iname '*~' \) -type f -exec bash -c '{ 
                echo "${@##*/}"
                cp -upv "$@" "$_SMB_FOLDER/REFs" 
                cp -upv "$_SMB_FOLDER/REFs/${@##*/}" "$@" 
            }' _ {} \;
        
        find ... -print0       # nul separator instead of NEWLINE; handles whitespaces; 

        find "$dir"            # abs-path default
        find .                 # rel-path default
        ...
        find "$_dir" -iname "foo*bar"  # find all @ subdirs within _dir & below
        ...
        find . -type d -not -path '*/.git/*' # exclude folder (dir) content

        # FOLDER SIZE of EACH `.git` folder; sum(s) of all subfolders therein ( + folder path) 
        ...
        # https://www.gnu.org/software/findutils/manual/html_mono/find.html#Format-Directives
            ...
            %Tk  # mtime per format 'k' ... 
                     ...
                     # %TD => 'YY/MM/DD'
                     # %TY-%Tm-%Td %TH:%TM => YYYY-MM-DD HH:MM
        ...
        # abs-path w/ newline q/ strong-quotes plus mod-time in 
        ...
        # find .... -exec ... {} ... ;
        #   where '{}' is expanded to the file currently processing
        #   '{}' may need to be single-quoted, and ';' may need to be escaped, i.e., '\;' 

            # cp : copy all target files in and under CURRENT DIR 
            # to "$@" folder; target is any fname @ REF.* except REF.*.lnk; 
            # uses Pattern Matching [glob]
        ...
        # https://www.gnu.org/software/findutils/manual/html_mono/find.html#Updating-A-Timestamp-File
            ...
            # -atime, -mtime, -ctime 
            find -atime +5 # find all accessed more than 5 days ago
            find -mtime -5 # find all modified [content] less than 5 days old

            # reset mtime of *.sh to that of its *.7z sibling, at all dirs hereunder 
            find . -iname '*.7z' -exec /bin/bash -c 'touch -r "$@" "$( find "${@%/*}" -iname '*.sh')"' _ {} \;

        # DELETE FOLDERs/FILEs ... history/journey on How To
        ...
        # find newer[XY]; m:modtime, a:accesstime, t:ref-is-literal-time-str[not-a-file]
        find -newermm REF_FILE # find files in current dir newer [modtime] than REF_FILE [modtime] 
            
        # find glob-set, *[SAMPLEs].url, and overwrite contents, per 'sed -i ...', in each file found ...
    ...
    # xARGS ('combine arguments'); pronounced EX-args
        ...
        #  per options AND machine capacity.
            ...
            xargs -P 10           # run [max=10] processes, CONCURRENTLY
       
       # require arg(s)
       ...|xargs --no-run-if-empty command

             |xargs echo aws ec2 ${action}-instances --instance-ids
        # handle WHITESPACE; implies ONE (first) ARG per line
        ...|xargs -I {} command {}       # '\n' [newline] delimited args
        ...|xargs -0 -I {} command _ {}  # '\0' [null] delimited args
        
            # E.g., find ... |xargs ... 
            # @ '\0'; NULL-DELIMITed + WHITESPACE in file-paths
            find . ... -print0 |xargs -0 command 
            
        # handle ARGS per STDIN or per PIPELINE ; tests if FD 0 [STDIN] is open ...
            [[ -t 0  ]] && { 
...
# TEXTUTILS [package]; Process text streams using filters ...
    ...
    jq  # like sed, but for JSON data : https://stedolan.github.io/jq/  
        jq [options...] filter [files...] 
            ...
            -M # Monochrome.
            -c '.[] | select(.genre | contains("house"))'

        # Example : List all values of the "CreatedAt" key of all docker volumes:
        ...
        # ... natively, sans jq 
         ...
         # SYNTAX: ".[]" is ARRAY operation; all nodes of all indices therein; "[0]" is first index
            ...
            # Filter and process an ARRAY of OBJECTS ...
                ...
                printf '{"a": [{"foo":"1", "bar":"2"},{"foo":"999", "bar":"77"}]}' |...
        ...
        # Functions
            ...
            ...|jq -c '.a[] | select(.foo | contains("999"))'  # Compact (vs Pretty Print)
                ...
                {"foo":"999","bar":"77"}  # Use -cM if sending to FILE, else CTRL (color) chars
    ...
    sed  # Stream EDitor; line-oriented text-file editor; "non-interactive", i.e., source file is unaffected 
        ...
        find . -iname '*.ovpn' -exec sed -i '$ a\foo' "{}" \+ # all files; VERY FAST
        echo 'foo' |tee *.ovpn > /dev/null  # alternate method using tee; VERY FAST
        ...
        sed '/^\s*$/d'    FILE  # delete all BLANK/EMPTY lines
        sed '/^[#]/d'     FILE  # delete all lines START w/ comment (#) only if it is FIRST CHAR
        sed '/^ *#/d'     FILE  # delete all lines START w/ comment (#) even w/ ONLY SPACES prepending
...
# FILE PROCCESSING UTILITIES
    ...
    # FILTER Chars/Lines
        cut -c 2,3,4,5 FILE  # filter [thru] those CHARS from EACH LINE of file
        cut -f 1 FILE        # filter out FIRST COLLUMN of file [if tab delimited]
        cut -f 1 -d : FILE   # filter out first collumn of file [if colon delimited]
    ...
    yes  # automatically approve all confirmations
    yes This string sent TO STDOUT REPEATEDLY UNTIL PROCESS KILLED 
    yes |sudo apt install PKG1 PKG2 ...
...
# FILE COMMANDS
    ...
    # mktemp : make ~ secure temp ...dir|file [and print path]
        # -d  = dir @ PWD or $TMPDIR if "tmp.XXXXXXXX" , 
        # -dt = dir @ $TMPDIR 
        # ['X' => 1 random char]
        _tmp_dir="$( mktemp -d "${0##*/}.XXXXXXXXXXXXX" )"           # mk dir @ $PWD
        _tmp_dir="$( mktemp -p "$TMP" -d "${0##*/}.XXXXXXXXXXXXX" )" # mk dir @ $TMP
         _tmp_file="$( mktemp )" # mk file @ $PWD
        printf "  %s\n  %s\n" " DIR @ '$_tmp_dir'" "FILE @ '$_tmp_file'" 

    # native linux replacement for mktemp
        _tmp_dir="$TMP/${0#*/}.$$.$PPID.$(date +%H.%M.%S.%N)"
        ( umask 077 && mkdir "$_tmp_dir" 2> /dev/null ) || { printf "FAIL @ mkdir" ; }

    # dd : iso => usb (bootable)
    dd if=input_file.iso of=/dev/sdb1 bs=512M; sync
