# ----------------------------------------------------------------
#  Potrace :: bitmap to SVG converter
#
#   Wikipedia      https://en.wikipedia.org/wiki/Potrace
#   SourceForge    http://potrace.sourceforge.net/#downloading
#   Man page       http://potrace.sourceforge.net/potrace.html
#
#   NetPBM Format  https://en.wikipedia.org/wiki/Netpbm_format
# ----------------------------------------------------------------
exit 

# USAGE; Potrace accepts PNM or BMP formats.
  ./potrace in.bmp -s --opaque -o out.svg
  ./potrace in.pnm -s --opaque -o out.svg

# If JPG/PNG/..., then first convert to PNM or BMP
  magick convert in.jpg out.pnm  # per ImageMagick 
  gm     convert in.jpg out.pnm  # per GraphicsMagick
  # alpha => black, @ png => pnm, so if desire otherwise, 
  # then convert accordingly, beforehand.
