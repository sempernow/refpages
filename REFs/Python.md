...
See `REF.Python.Docker` ([HTML](REF.Python.Docker.html "@ browser") | [MD](REF.Python.Docker.md))   
...
(.venv) $ touch app.py #... dev ...
```

## Build using wheel files (`.whl`)

Compiled binaries. They make for quicker subsequent builds, but they are less portable. 
For example, wheels built of Debian may not be supported by Alpine, 
yet the same application may install on Alpine without the wheel files built on Debian.
...
>Note: Building in a virgin or managed environment, such as a Docker container, removes the need for this hack
because all dependencies there are application dependencies.

Contrary to claims, there is no Python tool that lists all versioned dependencies of an application *and only those dependencies* in environments that are not virgin. Instead, what is generated by such tools is a list of all dependencies of *all modules of the current environment* regardless of those declared, e.g., in the `requirements.txt` file. This is true for all tools tested, including `pipreqs`.

However, there is a simple hack to solve this problem: Perform `pip` install twice on an *unversioned* module(s) list (`requirements.txt`) containing only the primary (known) module(s).  This forces `pip` to generate the list of application dependencies _and their versions_, from which a `requirements.txt` may be generated:
