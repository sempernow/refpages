...
# INIT 
    ...
    systemctl
    systemctl list-unit-files  # list all
...
# STORAGE 
    lsblk -o SIZE,LABEL,NAME,TYPE,FSTYPE,MOUNTPOINT,UUID 
...
# USERS & GROUPS
    ...
    # having UID:GID 777:777, 
    # having NO HOME DIRECTORY
    groupadd --gid 777 'foo'
    adduser --uid 777 --gid 777 --gecos "" --disabled-password --no-create-home 'foo'
...
# PROCESS MANAGEMENT [create/monitor/kill]
    ...
    ps      # Snapshot of current processes [syntax:UNIX|BSD|GNU]
        ...
        ps aux | wc # get number of processes [newlines words bytes]
    ...
    #  CPU(s) user-space:us, system:sy, idle:id, waiting:wa
        ...
        /bin/7za b # BENCHMARK [de]compression performance/speed [7-zip app]
...
# LOGGING 
    ...
    # journald generates BINARY log files; view via 
    
        journalctl 
            -b                     # boot info
            --since=yesterday      # everything since yesterday
        journalctl -u SERVICENAME # per service
        journalctl _COMM=su       # per command
        # or 
        systemctl status SERVICENAME

    # logrotate :: prevent logs from filling storage capacity
    /etc/logrotate.conf # manually edit 
    /etc/logrotate.d/   # include files; overrides logrotate.conf; rpm dumps such files here  
